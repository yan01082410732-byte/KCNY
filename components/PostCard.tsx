...
export function PostCard({ post, language }: { post: PublicPost; language: Language }) {
  ...
  return <article className="post-card">
    ...
    <div className="post-main">
      ...
      <div className="post-bottom"><div className="tags"><span>{postCategoryLabel(post.category, language)}</span><span>{postLanguageLabel(post.language, language)}</span><span>{commentCountLabel(post.comment_count, language)}</span></div><a className="post-read" href={postHref}>{language === "KR" ? "자세히 보기" : "查看详情"}</a></div>
    </div>
  </article>;
}
