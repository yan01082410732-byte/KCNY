...
export function PostCard({ post, language, authenticated, returnTo }: { post: PublicPost; language: Language; authenticated: boolean; returnTo: string }) {
  ...
  return <article className="post-card">
    ...
    <div className="post-main">
      ...
      <div className="post-bottom"><div className="tags"><span>{postCategoryLabel(post.category, language)}</span><span>{postLanguageLabel(post.language, language)}</span><span>{commentCountLabel(post.comment_count, language)}</span></div><div className="post-actions"><LikeButton postId={post.id} likeCount={post.likeCount} liked={post.likedByCurrentUser} language={language} returnTo={returnTo} authenticated={authenticated} /><BookmarkButton postId={post.id} bookmarked={post.bookmarkedByCurrentUser} language={language} returnTo={returnTo} authenticated={authenticated} /><a className="post-read" href={postHref}>{language === "KR" ? "자세히 보기" : "查看详情"}</a></div></div>
    </div>
  </article>;
}
