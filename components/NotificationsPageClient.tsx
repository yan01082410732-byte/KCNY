...
function formatNotificationTime(value: string, language: Language) {
...
export function NotificationsPageClient({
  ...
  return (
    <main>
      ...
      <section className="shell notifications-page">
        <div className="notifications-heading">
          ...
          <form action={markAllNotificationsRead}>
            ...
            <button className="button secondary" type="submit">{labels.markAll}</button>
          </form>
        </div>
          ...
          <div className="notification-list">
            {notifications.map((notification) => {
              ...
              const href = notification.type === "new_follower"
                ? `/u/${encodeURIComponent(actorUsername)}?lang=${language}`
                : notification.postId
                  ? `/posts/${encodeURIComponent(notification.postId)}?lang=${language}`
                  : `/notifications?lang=${language}`;

              return (
                <article className={`notification-card${notification.readAt ? "" : " unread"}`} key={notification.id}>
                  <a href={href}>
                    <strong>{actor?.displayName || actorUsername || "K"}</strong>
                    <p>{labels.message(notification.type, actorUsername || "K")}</p>
                  ...
                  {!notification.readAt && (
                    <form action={markNotificationRead}>
                      ...
                      <button type="submit">{language === "KR" ? "읽음" : "标为已读"}</button>
                    </form>
                  )}
                </article>
              );
