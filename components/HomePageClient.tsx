...
export function HomePageClient({ authenticated, username, displayName, posts }: { authenticated: boolean; username?: string; displayName?: string; posts: PublicPost[] }) {
  ...
  const [activeLanguage, setActiveLanguage] = useState<"All" | "CN" | "KR">("All");
  const text = copy[language];
  const shownPosts = activeLanguage === "All" ? posts : posts.filter((post) => post.language === activeLanguage);
  ...
  return <main>
    ...
    <section className="content shell" id="feed"><div className="section-heading"><div><p className="eyebrow"><span /> COMMUNITY</p><h2>{text.feed}</h2></div><a className="new-post" href={createHref}>＋ {text.create}</a></div><div className="filters"><button onClick={() => setActiveLanguage("All")} className={activeLanguage === "All" ? "active" : ""}>{text.all}</button><button onClick={() => setActiveLanguage("CN")} className={activeLanguage === "CN" ? "active" : ""}>{text.cn}</button><button onClick={() => setActiveLanguage("KR")} className={activeLanguage === "KR" ? "active" : ""}>{text.kr}</button></div><div className="feed">{shownPosts.map((post) => <PostCard key={post.id} post={post} language={language} />)}{shownPosts.length === 0 && <p className="empty">{text.empty}</p>}</div></section>
