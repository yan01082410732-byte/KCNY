create table if not exists public.profiles (
  ...
  preferred_language text not null default 'CN' check (preferred_language in ('CN','KR')),
...
begin
  ...
  values (new.id, coalesce(nullif(new.raw_user_meta_data->>'username',''), 'user_' || substr(new.id::text,1,8)), coalesce(nullif(new.raw_user_meta_data->>'display_name',''), 'KCNY user'), coalesce(nullif(new.raw_user_meta_data->>'preferred_language',''),'CN'));
  return new;
